...
def test_base_model_flow_truncates_invented_turns(monkeypatch):
    """Template-less base models (GPT-2 style) get "User:/Assistant:"
    framing, and the model then invents further turns that must be cut."""
    ...
    reply = server._chat_reply("gpt2", "What is the capital of France?")
    assert reply == "Paris."
