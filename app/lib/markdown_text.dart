...
/// blocks, lists, tables. Rendered as a flat string those markers read as
/// literal noise (`**Paris**` instead of a bold word), so the assistant bubble
/// routes its text through here instead of a plain [SelectableText]. Because
/// replies arrive complete rather than streaming into the bubble (see
...
/// `flutter_markdown` package behaved too.
class MarkdownText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final blocks = _buildBlocks(data, baseStyle);
    return Column(
...
/// gather a paragraph.
List<Widget> _buildBlocks(String data, TextStyle base) {
  ...
  while (i < lines.length) {
    ...
    if (heading != null) {
      widgets.add(_heading(heading.group(2)!.trim(), heading.group(1)!.length, base));
      i++;
      continue;
    }
...
Widget _paragraph(String text, TextStyle base) => SelectableText.rich(
      TextSpan(style: base, children: _inlineSpans(text, base)),
    );

Widget _heading(String text, int level, TextStyle base) {
...
Widget _blockquote(String text, TextStyle base) {
...
Widget _list(List<_ListItem> items, TextStyle base) => Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                Expanded(
                  child: SelectableText.rich(
                    TextSpan(style: base, children: _inlineSpans(it.content, base)),
                  ),
                ),
              ],
            ),
...
Widget _table(List<String> lines, TextStyle base) {
  ...
  // machinery doesn't report an intrinsic width, which throws under that. The
  // selectable bulk of a reply is its paragraphs and code, not table cells.
  Widget cell(String text, TextStyle style) => Padding(
        padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
        child: Text.rich(TextSpan(style: style, children: _inlineSpans(text, style))),
      );
...
/// bold). Anything unrecognized is emitted verbatim.
List<InlineSpan> _inlineSpans(String text, TextStyle base) {
  ...
  while (i < text.length) {
    ...
    // Link: [label](url). Without a URL launcher in the app the target isn't
    // opened, but the label is shown styled rather than as raw `[..](..)`.
    ...
    // Emphasis: ***/___, **/__, ~~, */_.
    final emph = _emphasisAt(text, i, base);
    if (emph != null) {
      flush();
...
class _Link {
  const _Link(this.label, this.end);
  final String label;
  final int end;
}
...
_Link? _linkAt(String text, int i) {
  ...
  return _Link(text.substring(i + 1, close), paren + 1);
}
...
_Emph? _emphasisAt(String text, int i, TextStyle base) {
  for (final spec in _emphSpecs) {
    ...
    while (true) {
      ...
      return _Emph(_inlineSpans(inner, _applyEmphasis(base, kind)), close + marker.length);
    }
  }
  return null;
}
